...
- **特点**：虚拟存储器时存储系统既具<font color='red'>有相当于外存的容量</font>又<font color='red'>有接近于主存的访问速度</font>
  好的，这是一个非常经典的考点。我们来详细解析虚拟存储器的特征。

#### 1.5.1.1 虚拟存储器的四个主要特征
...
#### 1.5.4.3 加速机构：<font color='red'>快表（TLB）</font>
...
#### 1.5.6.1 有效访问时间（EAT - Effective Access Time）
...
#### 1.6.2.1 寻道时间（Seek Time）- $T_s$
- **定义**：磁头移动到目标磁道所需的时间。
- **计算**：通常题目会给出<font color='red'>平均寻道时间</font>（如 5ms, 9ms）。这是由磁盘厂商测定的。

#### 1.6.2.2 旋转延迟（Rotational Latency）- $T_r$
...
#### 1.6.2.3 数据传输时间（Transfer Time）- $T_t$
...
**关键点**：传输大量连续数据时，$T_t$ 占比大；传输小文件时，$T_s$ 和 $T_r$ 是主要开销。
:::
---
...
| RAID级别            | 名称      | 关键技术                                         | 优点                                          | 缺点                                 | 应用场景         |
|:------------------|:--------|:---------------------------------------------|:--------------------------------------------|:-----------------------------------|:-------------|
| **RAID 0**        | 条带化     | 数据分散在多个磁盘                                    | <font color='red'>读写速度最快</font>             | <font color='red'>无冗余，可靠性最低</font> | 对性能要求高，数据不重要 |
| **RAID 1**        | 镜像      | 数据<font color='red'>完全复制到另一块盘</font>         | <font color='red'>100%冗余，可靠性高</font>        | 容量利用率仅50%，成本高                      | 对数据安全要求极高    |
| **RAID 5**        | 分布式奇偶校验 | <font color='red'>数据与奇偶校验信息交叉存储</font>在所有磁盘上 | 兼顾性能、容量、可靠性（允许坏1块盘）                         | 写性能有损失                             | 通用服务器存储      |
| **RAID 10 (1+0)** | 镜像+条带化  | 先做RAID1镜像对，再做RAID0条带化                        | <font color='red'>性能好，可靠性高</font>（每组镜像可坏1块） | 成本最高，容量利用率50%                      | 数据库、关键应用     |
| **RAID 01 (0+1)** | 条带化+镜像  | 先做RAID0，再整体镜像                                | 性能好，可靠性高                                    | 可靠性略低于RAID10                       | 较少使用         |
